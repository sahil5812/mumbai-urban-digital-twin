        ...
        <div className="flex items-center justify-between border-b border-white/10 pb-3">
          <div className="flex items-center gap-2 text-rose-400 font-bold">
            ...
            <h3 className="text-sm uppercase tracking-wider text-white glass-text-glow">
              {t("reportModalTitle", "Citizen Grievance & Pothole Portal")}
            </h3>
          </div>
          <button
            onClick={onClose}
          ...
          <div className="glass-panel-subtle border-emerald-500/40 rounded-2xl p-5 flex flex-col gap-3.5 text-center items-center shadow-[inset_0_1px_0_rgba(255,255,255,0.15)]">
            ...
            <div>
              <h4 className="text-sm font-bold text-emerald-300 drop-shadow-[0_0_8px_rgba(16,185,129,0.4)]">
                {t("ticketCreatedTitle", "Ticket Ingested into BMC Digital Twin!")}
              </h4>
              <p className="text-xs font-mono text-slate-300 mt-1 bg-white/[0.04] border border-white/10 px-2.5 py-1 rounded-lg">
                {t("ticketIdLabel", "Ticket ID")}: {response.ticket_id}
              </p>
            </div>
            <p className="text-xs text-slate-300 leading-relaxed">
              {t("ticketNotice", response.message)}
            </p>
            <button
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-3.5 text-xs">
            <div>
              <label className="text-slate-300 font-medium block mb-1">
                {t("reporterName", "Your Name / Mobile")}
              </label>
              <input
                type="text"
                placeholder={t("reporterNamePlaceholder", "e.g. Rahul Sharma (9820012345)")}
                value={name}
            ...
            <div className="grid grid-cols-2 gap-2.5">
              <div>
                ...
                >
                  <option value="POTHOLE">{t("catPothole", "Pothole / Road Damage")}</option>
                  <option value="WATERLOGGING">{t("catWaterlogging", "Severe Waterlogging")}</option>
                  <option value="DRAIN_BLOCKED">{t("catManhole", "Blocked Storm Drain")}</option>
                </select>
              </div>
              <div>
                <label className="text-slate-300 font-medium block mb-1">
                  {t("failureRiskLabel", "Severity Level")}
                </label>
                <select
                  value={severity}
                  ...
                  className="glass-input w-full rounded-xl p-2.5 text-slate-100 outline-none text-xs transition-all bg-slate-900/90"
                >
                  <option value="CRITICAL">{t("severityHigh", "Critical (Submerged)")}</option>
                  <option value="HIGH">{t("severityMedium", "High (Traffic Halted)")}</option>
                  <option value="MODERATE">{t("severityLow", "Moderate")}</option>
                </select>
              </div>
            </div>
            ...
            <div>
              <label className="text-slate-300 font-medium block mb-1">
                {t("landmarkLabel", "Landmark / Location")}
              </label>
              <select
                value={landmark}
              ...
              <span className="text-[10px] text-slate-500 mt-0.5 block font-mono">
                📍 {selectedCoords.lat.toFixed(4)}°N, {selectedCoords.lon.toFixed(4)}°E — {t("wardLabel", "Ward")} {selectedCoords.ward}
              </span>
            </div>

            <div>
              <label className="text-slate-300 font-medium block mb-1">
                {t("descriptionLabel", "Description / Details")}
              </label>
              <textarea
                rows={2}
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder={t("descriptionPlaceholder", "Describe the road or waterlogging condition...")}
            ...
            >
              ...
              <span>{isSubmitting ? t("submittingText", "Transmitting to Digital Twin...") : t("submitReportBtn", "Submit to BMC Command Center")}</span>
            </button>
          </form>
        )}
      </div>
